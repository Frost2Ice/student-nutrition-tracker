...
const openMeasures = computed(() =>
  open.value
    ? school.measuresForStudent(open.value.id).sort((a, b) => {
...
// Deep-link entry (focus-id): `open` is set during setup by the immediate
// watcher above, before this component mounts and before watch(open) is live,
// so neither render watcher fires. Render once on mount when already open.
onMounted(() => {
    ...
    <template v-if="open">
      <div class="panel">
        <div class="prof-head">
          ...
          <div class="prof-actions">
            ...
            <button v-if="!data.isReadonly" class="btn" @click="openEditStudent">แก้ไขข้อมูล</button>
            <button v-if="!data.isReadonly" class="btn danger" @click="delStudent = true">ลบ</button>
          </div>
        </div>
      ...
      <div class="panel">
        <div class="toolbar">
          ...
          <button v-if="!data.isReadonly" class="btn primary" style="min-height: 38px" @click="openAddMeasure">+ บันทึกการวัด</button>
        </div>
          ...
          <table>
            ...
            <tbody>
              <tr v-for="r in openMeasures" :key="`${r.year}-${r.term}-${r.round}-${r.savedAt}`">
                ...
                <td class="rowact">
                  <template v-if="!data.isReadonly && r.year === data.period.year">
      ...
      <template v-else>
        ...
        <template v-else>
          ...
          <div class="room-actions">
            <button v-if="!data.isReadonly" class="act-card" @click="importOpen = true">
          ...
          <div class="toolbar">
            ...
            <button v-if="!data.isReadonly" class="btn primary" style="min-height: 38px" @click="openAddStudent">+ เพิ่มนักเรียน</button>
          </div>
